...
for filename in allfiles:
    if filename[-4:]=='.jpg' or filename[-4:]=='.JPG' and len(filename)>4:
...
if len(filelist)==0:
    raise Exception("There aren't any .jpg files in this directory!")
                ...
                if event.type == QUIT or (event.type ==KEYDOWN and event.key==K_q):
                    ...
                    if event.type == KEYDOWN and event.key==K_SPACE and TL!= () and BR!=():
                        ...
                        if counter < len(newfiles)-1:
                            ...
                            pygame.display.set_caption("ColonyzerParametryzer - Press key to select format & save: 1536 (d), 384 (f), 96 (g), 48 (h)")
                            finished=True
                    if finished==True and event.type == KEYDOWN and (event.key==K_d or event.key==K_f or event.key==K_g or event.key==K_h):
                        if event.key==K_d:
                            Format='1536'
                        elif event.key==K_f:
                            Format='384'
